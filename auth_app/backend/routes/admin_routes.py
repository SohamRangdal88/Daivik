...
async def export_data(
    ...
    export_payload = {
        "report": "Daivik — Conversation Export",
    ...
    if format == "json":
        ...
        return StreamingResponse(
            ...
            headers={"Content-Disposition": f"attachment; filename=Daivik_export_{now_str}.json"},
        )
    else:  # txt
        W = 72
        lines = []
        lines.append("=" * W)
        lines.append("  Daivik — CONVERSATION EXPORT")
        lines.append("=" * W)
        ...
        return StreamingResponse(
            ...
            headers={"Content-Disposition": f"attachment; filename=Daivik_export_{now_str}.txt"},
        )
...
async def export_user_data(
    ...
    export_payload = {
        "report": f"Daivik — User Export: {user_info['name']}",
        "generated_at": now_utc.isoformat(),
        "total_messages": len(messages),
        **user_info,
        "conversation": messages,
    ...
    if format == "json":
        ...
        return StreamingResponse(
            io.BytesIO(content.encode("utf-8")),
            media_type="application/json",
            headers={"Content-Disposition": f"attachment; filename=Daivik_{safe_name}_{now_str}.json"},
        )
    else:
        W = 72
        lines = []
        lines.append("=" * W)
        lines.append(f"  Daivik — USER EXPORT: {user_info['name'].upper()}")
        lines.append("=" * W)
        lines.append(f"  Generated : {now_utc.strftime('%Y-%m-%d %H:%M:%S UTC')}")
        ...
        return StreamingResponse(
            io.BytesIO(content.encode("utf-8")),
            media_type="text/plain",
            headers={"Content-Disposition": f"attachment; filename=Daivik_{safe_name}_{now_str}.txt"},
        )
