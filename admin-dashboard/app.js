...
function mockData(path) {
  ...
  if (path.includes('/audit-log')) return Array.from({length:10},(_,i)=>({id:i+1,admin_email:'admin@Daivik.com',action:['user_enabled','user_disabled','user_deleted','conversations_deleted','user_enabled'][i%5],target_email:names[i%names.length].toLowerCase().replace(' ','.')+'@gmail.com',details:`Action on ${names[i%names.length]}`,timestamp:new Date(now-R(0,30)*864e5).toISOString(),ip_address:`103.21.${R(1,255)}.${R(1,255)}`}));
  return {};
}
  ...
  document.getElementById('app').innerHTML = `
    ${!hasAccess ? `<div id="login-screen">
      <div class="login-card">
        <div class="login-header"><h1>Daivik Admin</h1><p>Access restricted. Log in from the main bot page.</p></div>
        <a href="/" class="btn-primary" style="display:block;text-align:center;text-decoration:none;color:#fff">Go to DaivikAI Bot</a>
    ...
    <div id="app-layout" class="${hasAccess?'':'hidden'}">
      <aside class="sidebar">
        <a href="/" class="sidebar-brand" style="text-decoration:none;color:inherit"><div class="brand-mark">N</div><span>DaivikAdmin</span></a>
